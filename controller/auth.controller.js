...
//                 const token = jwt.sign({ id: results.insertId }, process.env.SECRET_KEY, { expiresIn: process.env.DURATION_TOKEN });
...
const userExists = (usuario, callback) => {
    const checkUserSql = "SELECT * FROM usuarios WHERE usuario = ?";
    
    db.query(checkUserSql, [usuario], (error, results) => {
        if (error) {
            return callback(error, null);
        }
        if (results.length > 0) {
...
const signIn = (req, res) => {
    ...
    userExists(usuario, (error, user) => {
        ...
        const validacion = bcrypt.compareSync(password, user.contrasena);
...
const signUp = (req, res) => {
    ...
    userExists(usuario, (error, exists) => {
        if (error) {
            return res.status(500).json({
                estado: "error",
                auth: false,
        ...
        if (exists) {
            ...
            const insertUserSql = "INSERT INTO usuarios (nombre, usuario, contrasena) VALUES (?, ?, ?)";

            db.query(insertUserSql, [nombre, usuario, passHasheado], (error, results) => {
                if (error) {
                    return res.status(500).json({
                        estado: "error",
                        auth: false,
                ...
                const token = jwt.sign({ usuario: usuario }, process.env.SECRET_KEY, { expiresIn: process.env.DURATION_TOKEN });
